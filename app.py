...
def send_message(message):
    ...
    message = client.messages.create(
        from_="whatsapp:+14155238886", body=message, to=user_phone_number
    )
...
def bot():
    ...
    trans_msg = (
        translator.translate(
            incoming_msg, dest="en", src=(user_lang if user_lang else "en")
        ).text
        if incoming_msg
        else ""
    )
    ...
    try:
        if similarity_score("exit", trans_msg) > 0.7:
            # clears places stack from MongoDB? clears user from mongoDB?
            response = (
                'Okay! You may start a new tour with "TOUR" or ask me some questions.'
            )
            response = translator.translate(
                response, dest=user_lang if user_lang else "en"
            ).text
            send_message(response)
            user_interest_populated = False
            location_populated = False
        elif location_populated and not user_interest_populated:
            ...
            five_attractions, loc_dict = generate_tour(
                user_addresses, user_phone_number, preference
            )
            # print(five_attractions)

            if five_attractions == "error" or five_attractions == []:
                ...
                chat_history.append("Tour attractions: " + " , ".join(five_attractions))
                response = translator.translate(
                    "One moment! Generating your tour . . . ", dest=user_lang
                ).text
                response = translator.translate(
                    response, dest=user_lang if user_lang else "en"
                ).text
                ...
                tour_message = translator.translate(
                    "Here is a summary of your tour: \n", dest=user_lang
                ).text
                ...
                tour_message += (
                    "\n\n"
                    + translator.translate(
                        "Message me when you're at the first location and when you're ready for the next. Feel free to ask me for more information about a certain location!",
                        dest=user_lang,
                    ).text
                )
        ...
        #   user location from WhatsApp is stored
            ...
            response = translator.translate(
                "Hello, I am your virtual tour guide, Tourista!",
                dest=user_lang if user_lang else "en",
            ).text
            response = translator.translate(
                response, dest=user_lang if user_lang else "en"
            ).text
            ...
            send_message(response)
            send_message(
                "Please type HELLO in the langauge you would like to communicate in."
            )

        elif similarity_score("hello", trans_msg) > 0.7 and not location_populated:
            ...
            make_request(user_phone_number, "post", "language", {"language": user_lang})
            ...
            user_update_location = make_request(
                user_phone_number, "patch", "location", {"location": user_loc}
            )
            ...
            similarity_score("here", trans_msg) > 0.7
            and location_populated
            and user_interest_populated
        ):
            attraction = remove_first(user_phone_number)
            if str(attraction) != "error":
                here_init_str = (
                    "Awesome! Here are some fun facts about " + str(attraction) + "."
                )
                area_query = (
                    "3 fun facts about"
                    + str(attraction)
                    + "in"
                    + user_addresses["city_address"]
                ...
                response = translator.translate(
                    here_init_str, dest=user_lang, src="en"
                ).text
                ...
                send_message(
                    translator.translate(area_info, dest=user_lang, src="en").text
                )

                send_message(
                    translator.translate(
                        "Ask me to go to the next location whenever you're ready!",
                        dest=user_lang,
                    ).text
                )
            ...
            similarity_score("next", trans_msg) > 0.7
            and location_populated
            and user_interest_populated
        ):
            if tour_done(user_phone_number):
                no_msg = (
                    "Your tour of "
                    + get_city(user_phone_number)
                    + " is complete! Thank you for using "
                )
                response = (
                    translator.translate(no_msg, dest=user_lang).text
                    + " Tourista! "
                    + translator.translate(
                        "Feel free to start a new tour with TOUR or ask me some questions.",
                        dest=user_lang,
                    ).text
                )
                user_interest_populated = False
                ...
                send_message(
                    translator.translate(
                        end_msg, dest=user_lang if user_lang else "en"
                    ).text
                )
        ...
        # knowledge base functionality
            ...
            response = qabot.run(
                ...
                + "My question: "
                + trans_msg
                + ". Only answer the question, and if there is Conversation history only use if relevant. Summarize reponse in less than 4 sentences in a cheerful tone."
            )
            response = translator.translate(
                response, dest=user_lang if user_lang else "en"
            ).text
            ...
            if similarity_score("skip places", trans_msg) > 0.6:
                ...
                msg2 = (
                    f"Sounds good! {spec_place} has been removed from your itinerary."
                )
            ...
            resume_msg = translator.translate(
                "Please let me know if you want to resume the tour.",
                dest=user_lang if user_lang else "en",
            ).text
            ...
            response = post_pschat_message(
                "Conversation history: "
                + str(chat_history)
                + "My question: "
                + trans_msg
                + ". Only answer the question in a cheerful tone, and if there is Conversation history only use if relevant. Summarize reponse in less than 4 sentences."
            )
            response = translator.translate(
                response, dest=user_lang if user_lang else "en"
            ).text
        ...
        if str(e) == "TouristaApi down":
            response = "Sorry Tourista API Down. Please try again later."
            send_message(response)
        else:
            raise
